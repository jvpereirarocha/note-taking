...
![We have a file called file.txt with 3 sentences. We’ve using the command sort with input redirect to sort the file.txt content](Input,%20Output%20and%20Redirection/Untitled.png)
...
![We’ve used the same command as you can see, the difference is that now we’ve saved the content of the command into another file called sorted-file.txt using the output redirect](Input,%20Output%20and%20Redirection/Untitled%201.png)
...
Using the >> we can apend new content to a existing file

![We’ve appended a new phrase “Hello, World” to the file.txt using the redirection syntax >>](Input,%20Output%20and%20Redirection/Untitled%202.png)
...
![Untitled](Input,%20Output%20and%20Redirection/Untitled%203.png)

Now let’s save the command content in two different files. A `result.txt` and `err.txt`

![When using the standard output (stdout) I/O we can omit the 1> syntax and use only > as you can see in the command >result.txt](Input,%20Output%20and%20Redirection/Untitled%204.png)
...
![The stdout and stderr are now in the content.txt file. We combined the two I/O with the syntax >content.txt 2>&1 where 2>&1 is used to tell to the system that we want to save stderr(2) and stdout(1) in the same file (content.txt)](Input,%20Output%20and%20Redirection/Untitled%205.png)
...
![The old content wasn’t overrated by the I/O command](Input,%20Output%20and%20Redirection/Untitled%206.png)
...
![Untitled](Input,%20Output%20and%20Redirection/Untitled%207.png)

Now, in the second example, we want to save in the `fail.txt` the `stderr` content

![As mentioned early we can omit the 1>/dev/null in this case. The syntax >/dev/null would the same effect here](Input,%20Output%20and%20Redirection/Untitled%208.png)

As mentioned early we can omit the 1>/dev/null in this case. The syntax >/dev/null would the same effect here

**Throwing away stdout and stderr**

Let’s throw away the `stdout` and the `stderr` I/O

![Untitled](Input,%20Output%20and%20Redirection/Untitled%209.png)

![image.png](Input,%20Output%20and%20Redirection/image.png)